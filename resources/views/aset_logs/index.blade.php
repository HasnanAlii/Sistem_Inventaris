        ...
        <div class="max-w-full mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-md rounded-2xl overflow-hidden border border-gray-100">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 p-6 border-b border-gray-200 
                    bg-gradient-to-r from-blue-50 to-sky-50 rounded-t-2xl shadow-sm">
                ...
                <!-- ✅ Pesan Sukses -->
                @if(session('success'))
                    <div class="p-4 mx-6 my-4 text-blue-800 bg-blue-50 border border-blue-200 rounded-lg shadow-sm text-base">
                        {{ session('success') }}
                    </div>
                @endif
                        ...
                        <tbody>
                            @forelse($logs as $log)
                                <tr class="hover:bg-blue-50 transition duration-150">
                                    ...
                                    <td class="px-5 py-3 text-gray-700">{{ $log->nama_barang }}</td>
                                    <td class="px-5 py-3 text-center text-gray-700">{{ $log->jumlah }}</td>
                                    <td class="px-5 py-3 text-center text-gray-700">{{ number_format($log->biaya, 0, ',', '.') }}</td>
                                    <td class="px-5 py-3 text-center text-gray-600">{{ $log->tanggal_pengadaan?->format('d/m/Y') ?? '-' }}</td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="text-center text-gray-500 py-6">
                                        Belum ada riwayat pengadaan aset.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
