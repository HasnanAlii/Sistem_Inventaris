<x-app-layout>
            ...
            <div class="bg-white shadow-md rounded-xl overflow-hidden">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-center gap-4 p-6 border-b border-gray-200">
                    ...
                    <a href="{{ route('lokasis.create') }}"
                       class="bg-purple-600 text-white px-4 py-2 rounded-lg hover:bg-purple-700 w-full sm:w-auto text-center">
                        + Tambah Lokasi
                    </a>
                    @endhasrole
                    ...
                    <table class="w-full text-left border border-gray-200 rounded-lg">
                        <thead class="bg-gray-100 text-gray-700">
                            <tr>
                                ...
                                <th class="px-4 py-2 border">Lantai</th>
                        ...
                        <tbody>
                            @forelse ($lokasis as $i => $lokasi)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-2 border">{{ $lokasi->nama }}</td>
                                    <td class="px-4 py-2 border">{{ $lokasi->kode }}</td>
                                    <td class="px-4 py-2 border">{{ $lokasi->gedung }}</td>
                                    <td class="px-4 py-2 border">{{ $lokasi->lantai }}</td>
                                    <td class="px-4 py-2 border">{{ $lokasi->keterangan ?? '-' }}</td>
                                    @hasrole('petugas')
                                    <td class="px-4 py-2 border text-center">
                                        <div class="flex justify-center space-x-2">
                                            <a href="{{ route('lokasis.edit', $lokasi) }}" class="text-yellow-600 hover:underline">Edit</a>
                                            <button type="button" 
                                                    onclick="openDeleteModal('{{ route('lokasis.destroy', $lokasi) }}')" 
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-4 py-4 text-center text-gray-500">Belum ada lokasi.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
        <div class="bg-white rounded-lg p-6 w-full max-w-sm shadow-lg">
            <h3 class="text-lg font-semibold mb-3">Hapus Lokasi?</h3>
